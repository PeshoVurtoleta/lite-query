...
| `@zakkster/lite-stream` | 1.1.0 | published 2026-07-10; additive `toAsyncIterable` enrichments; `^1.0.0` peer admits it; suite green against it. Roadmap enriched 2026-09-01 (`../LiteStream/ROADMAP.md`, LS1..LS4): its LS-01 (dispose-mid-pump pulls forever; llms claim false since 1.0.0) verified NOT to bite us -- `streamStop` aborts before any teardown (`StreamQuery.js:156`); its LS3 v1.3.0 targets our hand-rolled ring (see parking ledger). UPDATE 2026-09-02: lite-stream is now **1.3.0** -- LS1..LS3 all shipped (their harness caught and fixed a real pump bug, LS-13, on the way); `pipeToSignal` gained `mode`/`maxBuffer`, `droppedCount`/`overflowCount` getters on the stop fn, an `onValue` tap, and `onAbort` (abort vocabulary pinned in their `decisions/0001`); their s7 conformance tier freezes OUR exact 1.1.x call shape and proves a rewrite-parity case. The collapse rider is folded into Q3. LS4 (1.4.0) stays GATED on Q8's spike; SPIKE INPUTS recorded in the Q8 brief. **Q8 SPIKE VERDICT (2026-09-02, ratified ON-2):** push-writer SURVIVES -- a hand-made async queue feeding `pipeToSignal` allocates one promise + one resolver per frame on the follower's warm path (falsified on law 4, not taste), so `createSignalWriter` is REQUIRED in lite-stream and LS4 shrinking to it alone is the right 1.4.0. idleTimeout -> lite-query's in-package watchdog (lite-stream cannot observe channel traffic, so it would arm on the wrong side of the failure); standalone version stays parked their side. share/refcount -> STRUCK (no F1-F7 cell yields one-iterator->N-local-consumers). SEQUENCING: Q8 proceeded in PARALLEL behind the single `projectFrame` seam, its semantics pinned by a differential parity test whose oracle is lite-stream's live `pipeToSignal`; when LS4 ships `createSignalWriter` the seam swaps and the test does not change. The parity assertion + 12-case corpus were handed to LS4 for its s7 (operator relays; recorded on both sides). Transport wording pinned: "followers receive frames as BroadcastChannel messages over the existing crossTab channel; lite-channel supplies only the caller-wired isLeader oracle." |
...
status: shipped (2026-09-02; ladder 474c512..ab63ab7: C1-C6 + reviewer doc pass 574edb6 + QA-defect fix ab63ab7; reviewer APPROVED w/3 LOW all resolved-or-probed, QA FAIL(3: getNextCursor-throw wedge, setQueryData non-array divergence, prefetch-on-infinite page advance) -> fixed w/pinned contracts -> QA PASS; suite 203, GATE byte-identical, 4/4 controls; drill sites=3/16 tests=203/0 GATE ok pack 63.5 kB -- the drill itself falsified and collision-proofed the /await VERSION provenance guard; operator publish 855214e; registry artifact verified: 13 files, [1.3.0] head, VERSION sync)
