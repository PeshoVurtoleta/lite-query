...
> Historical document: this is the 1.1.0 release-candidate spec, preserved for
> provenance. For current status and the forward plan, see ROADMAP.md.
...
The forward plan is no longer maintained here. See ROADMAP.md for the current,
session-by-session roadmap (devtools, focus/reconnect triggers, SSR hydration,
infinite-query helper, and the 2.0 shared-stream work).
