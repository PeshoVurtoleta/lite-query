...
### Added -- `test/24-signalbox_test.mjs` activated (9 tests)
...
- **Full suite green** against the 1.5.0 engine: 439 tests, 438 pass, 0 fail,
  ...
  `test/27-create-root_test.mjs`, +3 box-coverage tests). **Coverage** (c8@11,
...
- **Full suite green** against the 1.4.0 engine: 430 tests, 420 pass, 0 fail,
...
- **`stats()` shape** is now the authoritative 11-key 1.4.x shape; the
  introspection-contract test is updated to expect the three counters present
  (the 1.2.x/1.3.0 test pinned their *absence*; that pin is correctly inverted
  for 1.4.0).
...
- **Full suite green** against the 1.3.0 engine: 424 tests, 414 pass, 0 fail,
  1 architecturally-N/A SSR skip (the `24-signalbox` suite is `{skip:true}` on
  1.3.x -- those primitives land in 1.5.0). The eager-default flip changed no
  test outcome.
